param(
    ...
    [ValidateSet('android-exe','android-shared','static-nolibc')][string]$Mode = 'android-exe',
    [string]$ExpectedLoadAlign = '0x4000',
    [string]$ExpectedRelroEndAlign = '0x4000',
    [string[]]$AllowedNeeded = @('libc.so','libdl.so'),
    [switch]$RequireLibc
)
...
function Test-RelroAligned {
    ...
    foreach ($relro in $relroLines) {
        ...
        if (($relroEnd % $alignValue) -ne 0) {
            ...
            Fail "$Binary : GNU_RELRO end $endHex is not aligned to $ExpectedAlign"
        }
    }
}
...
# Allow callers to pass AllowedNeeded either as a real PowerShell string array
# or as one CSV token. When this verifier is invoked through an ArgList array
# to powershell.exe, multiple unquoted values after -AllowedNeeded are not
# reliably bound as a string[] on Windows PowerShell 5.1. Normalizing here
# keeps the verifier deterministic.
$AllowedNeeded = @(
    $AllowedNeeded | ForEach-Object {
        $_ -split '[,;]'
    } | ForEach-Object {
        $_.Trim()
    } | Where-Object {
...
if ($Mode -eq 'android-exe') {
    ...
    $badLibs = @($libs | Where-Object { $AllowedNeeded -notcontains $_ })
    if ($RequireLibc -and -not ($libs -contains 'libc.so')) { Fail 'binary must depend on libc.so' }
    ...
    Write-Host ("[VERIFY-OK] mode=android-exe Type=DYN INTERP=/system/bin/linker64 NEEDED=" + ($libs -join ',') + " LOAD_ALIGN=$ExpectedLoadAlign RELRO_END_ALIGN=$ExpectedRelroEndAlign")
    exit 0
}

if ($Mode -eq 'android-shared') {
    if ($typeText -notmatch '\bDYN\b') { Write-Host $typeText; Fail 'shared library Type must be DYN' }
    if (@($phdr | Select-String 'INTERP').Count -gt 0) { Fail 'shared library must not have INTERP' }
    if ($dynRc -ne 0) { $dyn | ForEach-Object { Write-Host $_ }; Fail 'readelf -d failed' }
    $badLibs = @($libs | Where-Object { $AllowedNeeded -notcontains $_ })
    if ($badLibs.Count -gt 0) {
        Write-Host 'NEEDED entries:'
        if ($needed.Count -eq 0) { Write-Host '  <none>' } else { $needed | ForEach-Object { Write-Host $_.Line } }
        Fail ('shared library has unexpected NEEDED dependencies: ' + ($badLibs -join ', '))
    }
    Write-Host ("[VERIFY-OK] mode=android-shared Type=DYN INTERP=<none> NEEDED=" + ($libs -join ',') + " LOAD_ALIGN=$ExpectedLoadAlign RELRO_END_ALIGN=$ExpectedRelroEndAlign")
    exit 0
}
